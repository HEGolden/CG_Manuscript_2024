...
Introduction:
Here I use the R package MCMCglmm to fit generalized linear mixed models to our 2017 common garden length at hatch and swimup response variables because a range of distributions are supported and several types of variance structure for the random effects and residuals can be fitted. The function uses a Bayesian approach that incorporates terminology for fixed and random effects.
...
Here we use length at hatch, length at swimup and yolk volume at hatch as response variables to investigate the heritability of these traits.

## Length at Hatching
The first data set is length (mm) at hatching.
```{r LoadData}
...
This section follows the Vingnette for the 'fullfact' R package, which was created specifically to calculate additive genetic variation, such as from common garden experiments.

```{r Vignette_fullfact}
library(fullfact)
```

I use the same subsets of the data created from the 'hatch' data above to calculate the narrow sense heritability for all populations and treatments separately.


Kuparuk, Treatment = T1 (8 degrees)
```{r T1Kup}
mod_t1kup <- observLmer(observ = t1kup, dam = "dam", sire = "sire", response = "length")
mod_t1kup

```

```{r T1Kup_h2}
t1kup_h2 = mod_t1kup$calculation[1,2]/(mod_t1kup$calculation[1,2] + mod_t1kup$calculation[2,2])
t1kup_h2
```



Kuparuk, Treatment = T2 (12 degrees)
```{r t2Kup}
mod_t2kup <- observLmer(observ = t2kup, dam = "dam", sire = "sire", response = "length")
mod_t2kup

```

```{r t2Kup_h2}
t2kup_h2 = mod_t2kup$calculation[1,2]/(mod_t2kup$calculation[1,2] + mod_t2kup$calculation[2,2])
t2kup_h2
```


Kuparuk, Treatment = T3 (16 degrees)
```{r t3Kup}
mod_t3kup <- observLmer(observ = t3kup, dam = "dam", sire = "sire", response = "length")
mod_t3kup

```

```{r t3Kup_h2}
t3kup_h2 = mod_t3kup$calculation[1,2]/(mod_t3kup$calculation[1,2] + mod_t3kup$calculation[2,2])
t3kup_h2
```
...
sagavanirktok, Treatment = T1 (8 degrees)
```{r t1sag}
mod_t1sag <- observLmer(observ = t1sag, dam = "dam", sire = "sire", response = "length")
mod_t1sag

```

```{r t1sag_h2}
t1sag_h2 = mod_t1sag$calculation[1,2]/(mod_t1sag$calculation[1,2] + mod_t1sag$calculation[2,2])
t1sag_h2
```



sagavanirktok, Treatment = T2 (12 degrees)
```{r t2sag}
mod_t2sag <- observLmer(observ = t2sag, dam = "dam", sire = "sire", response = "length")
mod_t2sag

```

```{r t2sag_h2}
t2sag_h2 = mod_t2sag$calculation[1,2]/(mod_t2sag$calculation[1,2] + mod_t2sag$calculation[2,2])
h2_guess = (0.1947529 + 0.1947529)/(0.56582239 + 0.1947529 + 0.1947529)
t2sag_h2
h2_guess
```


sagavanirktok, Treatment = T3 (16 degrees)
```{r t3sag}
#mod_t3sag <- observLmer(observ = t3sag, dam = "dam", sire = "sire", response = "length")
#mod_t3sag
...
```{r t3sag_h2}
#t3sag_h2 = mod_t3sag$calculation[1,2]/(mod_t3sag$calculation[1,2] + mod_t3sag$calculation[2,2])
#t3sag_h2
```
